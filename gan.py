...
def train_generator(batch_size):
    ...
    # ...
    fake_img = generator(noise, labels).to(device)
    fake_labels = discriminator(fake_img, labels).to(device)
    loss = loss_function(fake_labels, torch.ones_like(fake_labels).to(device)).to(device)
    loss.backward()
...
def train_discriminator(batch_size, images, labels=None): # labels to be used in 5.4.
    ...
    # print("image shapes fake img:", fake_img.shape, "real img:", images.shape)

    fake_labels = discriminator(fake_img.detach().to(device), labels).to(device)
    real_labels = discriminator(images.view(batch_size, 784).to(device), labels).to(device)

    fake_loss = loss_function(fake_labels, torch.zeros_like(fake_labels).to(device)).to(device)
    real_loss = loss_function(real_labels, torch.ones_like(real_labels).to(device)).to(device)

    real_loss.backward()
    fake_loss.backward()

    loss = (real_loss + fake_loss).to(device)
    ...
    for batch, (imgs, labels) in enumerate(train_loader):
        ...
        if ((batch + 1) % 500 == 0 and (epoch + 1) % 1 == 0):
            ...
            with torch.no_grad():  # disables gradient computation to speed things up
                ...
                # plt.show()
                fig.savefig(f"./resultsCGAN/CGAN_Generations_Epoch_{epoch}")
                print(
